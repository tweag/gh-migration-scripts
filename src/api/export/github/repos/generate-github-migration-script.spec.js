...
describe('generateGithubMigrationScript', () => {
	beforeEach(() => {
	...
	test('it should generate GHES migration script and save to file', async () => {
		...
		await generateGithubMigrationScript(mockOptions);
		...
		expect(succeedMock).toHaveBeenCalledWith(
			'Completed generation of migration script',
		);

		// Check if getScript is called for each row
		for (const row of mockRows) {
			expect(utils.getScript).toHaveBeenCalledWith(
				row.repo,
				row.visibility,
				mockOptions,
			);
		...
		expect(utils.saveScriptToFile).toHaveBeenCalledWith(
			mockScriptsArr,
			mockOptions,
		);
	});
