...
describe('deleteRepos', () => {
  ...
  test('it should delete repos and generate status output', async () => {
    ...
    utils.getData.mockImplementationOnce(() => Promise.resolve(mockRepositories));
    ...
    expect(global.console.log).toHaveBeenCalledWith({ repo: 'repo1', status: 200, statusText: 'OK', errorMessage: '' });
    expect(global.console.log).toHaveBeenCalledWith({ repo: 'repo2', status: 404, statusText: 'Not Found', errorMessage: 'Mock deletion error response for repo2' });

    // Check if getStringifier is called with the correct file name and columns
    expect(utils.getStringifier).toHaveBeenCalledWith('mockOrg-delete-repos-status-mockCurrentTime.csv', ['repo', 'status', 'statusText', 'errorMessage']);

    // Check if stringifier.write is called with the expected data for each repo
    expect(utils.getStringifier().write).toHaveBeenCalledWith({ repo: 'repo1', status: 200, statusText: 'OK', errorMessage: '' });
    expect(utils.getStringifier().write).toHaveBeenCalledWith({ repo: 'repo2', status: 404, statusText: 'Not Found', errorMessage: 'Mock deletion error response for repo2' });
